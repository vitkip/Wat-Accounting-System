...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!checkCSRF()) {
        ...
        header('Location: ' . BASE_URL . '/modules/income/add.php');
        exit();
    }
    ...
    if (empty($errors)) {
        try {
            ...
            if ($currentTempleId) {
                $sql = "INSERT INTO income (temple_id, date, description, amount, category, created_by) 
...
// ດຶງໝວດໝູ່ລາຍຮັບຕາມວັດ (ລວມໝວດໝູ່ທົ່ວໄປ)
if ($currentTempleId && function_exists('getTempleIncomeCategories')) {
    // ດຶງໝວດໝູ່ຂອງວັດນີ້ + ໝວດໝູ່ທົ່ວໄປ (temple_id IS NULL)
    $categories = getTempleIncomeCategories($currentTempleId);
} else {
    ...
    <div class="mb-6 sm:mb-8 page-header">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="w-full sm:w-auto">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">ເພີ່ມລາຍຮັບ</h1>
                <p class="text-sm sm:text-base text-gray-600">ບັນທຶກລາຍຮັບເຂົ້າລະບົບ</p>
            </div>
    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow-md p-4 sm:p-8 form-card">
        <form method="POST" action="">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6 form-grid">
                ...
                <div>
                    ...
                    <input type="date" 
                           ...
                           class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent form-input text-base">
                </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 button-group">
                ...
                <button type="submit" 
                        class="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition duration-200">
                    ບັນທຶກ
                </button>
            </div>
