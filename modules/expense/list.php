...
// Get Total
$stmt = $db->prepare("SELECT COUNT(*) as total FROM expense WHERE {$whereClause}");
$stmt->execute($params);
...
// Get Total Amount
$stmt = $db->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM expense WHERE {$whereClause}");
$stmt->execute($params);
                ...
                <?php else: ?>
                    ...
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <a href="<?php echo BASE_URL; ?>/modules/expense/edit.php?id=<?php echo $record['id']; ?>" 
                               class="text-blue-600 hover:text-blue-900 mr-3">ແກ້ໄຂ</a>
                            <a href="#" 
                               onclick="confirmDeleteExpense(<?php echo $record['id']; ?>); return false;"
