...
try {
    if ($currentTempleId) {
        ...
        $stmt = $db->prepare("SELECT * FROM expense_categories WHERE id = ? AND temple_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            if ($currentTempleId) {
                $stmt = $db->prepare("SELECT id FROM expense_categories WHERE name = ? AND temple_id = ? AND id != ?");
                $stmt->execute([$name, $currentTempleId, $id]);
            } else {
                // ກວດສອບໝວດໝູ່ທົ່ວໄປ (temple_id IS NULL)
                $stmt = $db->prepare("SELECT id FROM expense_categories WHERE name = ? AND temple_id IS NULL AND id != ?");
