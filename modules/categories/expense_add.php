    ...
    if (empty($errors)) {
        try {
            ...
            if ($currentTempleId) {
                ...
                $stmt = $db->prepare("SELECT id FROM expense_categories WHERE name = ? AND temple_id IS NULL");
                $stmt->execute([$name]);
            }
            ...
            if ($currentTempleId) {
                ...
                $stmt = $db->prepare("INSERT INTO expense_categories (temple_id, name, description) VALUES (NULL, ?, ?)");
